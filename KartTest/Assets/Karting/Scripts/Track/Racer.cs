...
namespace KartGame.Track
{
    ...
    public class Racer : MonoBehaviour, IRacer
    {
        ...
        //追加
        public GameObject rewindManeger;
        ...
        {
            //変更
            if (m_CurrentLap > 0 && !m_IsTimerPaused && rewindManeger.GetComponent<RewindManager>().isRecording())
            {
                m_Timer += Time.deltaTime;
            }
            else if (rewindManeger.GetComponent<RewindManager>().isRecording() == false)
            {
                m_Timer -= Time.deltaTime;
            }
        }
